...
,
    ...
    },
    {
      ...
      keywords: ["네트워크 드라이브 안됨", "공유 폴더 접근 오류", "네트워크 드라이브 끊김", "Z드라이브 없어짐", "공유 폴더 경로 오류", "시놀로지 나스 접속 안됨", "윈도우 업데이트 후 나스 접속 안됨", "계정 또는 암호가 잘못됨", "오류 1208"],
      causes: ["로그인 전 네트워크 드라이브 재연결 실패", "자격 증명 만료 또는 변경", "SMB 버전 불일치", "방화벽 또는 검색 설정 비활성화", "Windows 11 24H2 이후 SMB 서명(signing) 강제·게스트 폴백 비활성화로, 계정·비밀번호 없이 접속하던 NAS 공유가 차단되는 경우", "실제 계정 인증을 쓰고 있어도, Windows에 캐시된 예전 비밀번호가 계속 재시도되며 NAS의 로그인 실패 차단(계정 보호/Auto Block) 기능을 건드려 정상 비밀번호까지 거부되는 경우"],
      checks: ["네트워크 검색과 파일 공유가 켜져 있는지 확인하세요(고급 공유 설정).", "자격 증명 관리자에서 해당 서버 자격 증명을 삭제하고 재입력하세요.", "드라이브 매핑 시 '로그온 시 다시 연결'과 '다른 자격 증명 사용'을 함께 체크하세요.", "SMB1 프로토콜이 필요한 구형 기기라면 선택적 기능에서 SMB 1.0 활성화를 검토하세요.", "NAS(시놀로지 등)의 공유 폴더가 게스트·익명 접근으로 설정돼 있다면, NAS 쪽에서 실제 계정+비밀번호로 접근하도록 권한을 바꾸는 것이 근본 해결책입니다.", "이미 실제 계정으로 설정돼 있는데도 거부된다면, NAS의 계정 보호(Auto Block) 설정에서 해당 계정·IP가 차단됐는지 확인하세요(시놀로지: 제어판 → 보안 → 계정)."],
    ...
    "network-drive-error": {
      ...
      decision: [
        ...
        },
        {
          ...
          text: "계정·비밀번호는 정상이라는 뜻이므로 비밀번호를 계속 바꿔봐야 소용없습니다. 자격 증명 관리자에서 저장된 항목을 삭제하고 다시 시도하고, 그래도 안 되면 NAS의 SMB 프로토콜·서명 설정과 계정 보호(Auto Block)가 SMB 로그인만 차단하고 있지 않은지 확인하세요."
        }
      ],
      examples: [
      ...
      faq: [
        ...
        },
        {
          ...
          a: "웹 로그인과 SMB 네트워크 드라이브 인증은 별개의 경로라서, 웹 로그인이 된다면 계정·비밀번호 자체는 문제가 아닙니다. Windows 자격 증명 캐시, SMB 프로토콜·서명 설정, 또는 SMB 로그인만 별도로 차단하는 계정 보호 설정을 확인하세요."
        }
      ]
    },
    "store-error": {
