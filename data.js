window.SITE_DATA = {
  ...
  symptoms: [
    ...
    },
    {
      ...
      title: "AMD CPU 쿨러 장착 후 POST 실패",
      overview: "AMD CPU를 사용하는 PC에서 사제 공랭·수랭 쿨러를 장착한 뒤 화면이 나오지 않거나 CPU·DRAM 디버그 LED에서 멈추는 증상을 다룹니다. Ryzen 7에만 한정된 현상이나 AMD가 공식 확인한 공통 결함으로 단정할 수는 없습니다. 다만 AM4·AM5용 장착 키트, 백플레이트, 스페이서가 맞지 않거나 나사를 한쪽부터 과도하게 조여 메인보드가 휘고 접촉 상태가 달라지면 POST 실패처럼 보일 수 있습니다. 쿨러를 느슨하게 둔 채 사용하는 대신 제조사 설명서대로 균등하게 재장착하고, 증상이 반복되면 장착 부품을 교차 확인해야 합니다.",
      summary: "AMD CPU 쿨러를 장착하거나 조인 뒤 화면이 없고 CPU·DRAM LED에서 POST가 멈추는 경우",
      keywords: ["amd cpu 부팅 안됨", "라이젠 부팅 안됨", "am4 쿨러 장력", "am5 쿨러 장력", "사제 쿨러 부팅 안됨", "공랭 쿨러 부팅 안됨", "쿨러 볼트 꽉 조이면 부팅 안됨", "쿨러 나사", "cpu led", "dram led", "post 실패", "메모리 채널 인식", "백플레이트", "스페이서", "쿨러 장착 압력"],
      causes: [
...
,
  ...
  symptomDetails: {
    ...
    "amd-cpu-cooler-pressure-no-post": {
      ...
      intro: [
        "사제 공랭 또는 수랭 쿨러를 장착한 뒤 전원은 들어오지만 화면이 나오지 않고 POST가 끝나지 않는다면, CPU 고장으로 바로 단정하기보다 장착 과정에서 달라진 조건을 먼저 되돌려 봐야 합니다. 이 증상은 Ryzen 7만의 문제로 확인된 것이 아니며 AM4·AM5 기반 AMD CPU 시스템에서 장착 키트와 조임 상태가 맞지 않을 때 관찰될 수 있는 점검 유형입니다.",
        "CPU 쿨러는 방열판을 CPU에 밀착시키는 부품이므로 적정한 압력이 필요합니다. 그러나 잘못된 스페이서나 백플레이트를 사용하거나 한쪽 나사를 먼저 끝까지 조이면 메인보드가 불균일하게 휘고, 소켓 접촉 또는 메모리 초기화 상태에 영향을 줄 가능성이 있습니다. 이 설명은 가능한 작동 원인을 정리한 것이며 모든 AMD CPU의 공통 결함을 뜻하지 않습니다.",
      ...
      mistakes: [
        "Ryzen 7 또는 AMD CPU의 공통 결함이라고 단정하는 것",
      ...
      faq: [
        {
          q: "AMD Ryzen 7에서만 생기는 알려진 결함인가요?",
          a: "확인된 범위에서는 Ryzen 7 전체에 공통된 공식 결함으로 볼 근거가 없습니다. CPU 세대보다 쿨러 장착 부품, 메인보드, 조임 방식과 소켓 접촉 상태를 함께 확인해야 합니다."
        },
        {
