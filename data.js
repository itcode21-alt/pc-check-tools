window.SITE_DATA = {
  ...
  symptoms: [
    ...
    },
    {
      ...
      title: "듀얼 모니터 DP 인식 불가",
      summary: "DisplayPort(DP)로 연결한 두 번째 모니터가 감지되지 않거나 신호 없음으로 표시되는 경우",
      keywords: ["듀얼모니터 dp 인식 안됨", "듀얼 모니터 dp 연결 안됨", "dp 포트 인식 불가", "displayport 인식 안됨", "두번째 모니터 안나옴", "보조 모니터 신호없음", "윈도우 모니터 감지 안됨", "dp 듀얼모니터", "displayport no signal", "microsoft display detect", "mst 모니터 인식"],
      causes: ["모니터의 입력 소스가 DP가 아니거나 DP 케이블·그래픽카드 포트 한 경로에만 문제가 있는 경우", "두 모니터를 함께 연결했을 때 해상도·주사율·HDR·DSC 설정 또는 도킹 스테이션의 대역폭이 부족한 경우", "절전 복귀 또는 그래픽 드라이버 업데이트 후 Windows의 확장 화면 설정과 그래픽 드라이버 상태가 꼬인 경우", "노트북 USB-C/Thunderbolt 도킹 환경에서 DP Alt Mode·MST 지원 또는 도킹 전원 공급이 부족한 경우"],
      checks: ["각 모니터를 같은 DP 케이블·같은 그래픽카드 포트에 하나씩 연결해 모두 단독 출력되는지 먼저 확인하세요.", "두 번째 모니터 OSD 메뉴에서 입력 소스를 연결한 DP 또는 Auto로 설정하고, 케이블을 그래픽카드와 모니터 양쪽에서 다시 체결하세요.", "Windows 키 + P에서 '확장'을 선택한 뒤 설정 > 시스템 > 디스플레이의 '여러 디스플레이'에서 '감지'를 실행하세요.", "두 화면이 각각 정상인데 함께 연결할 때만 실패하면 두 모니터의 해상도·주사율을 낮춰 테스트하고, 도킹·허브를 빼고 그래픽카드 DP 포트에 직접 연결해 비교하세요.", "그래픽 드라이버를 GPU 또는 노트북 제조사 지원 페이지 기준으로 업데이트하거나, 업데이트 직후 시작됐다면 이전 드라이버로 되돌려 재현 여부를 확인하세요."],
  ...
  symptomDetails: {
    ...
    "dual-monitor-dp-not-detected": {
      badge: "디스플레이",
      subtitle: "DP로 연결한 두 번째 모니터가 보이지 않을 때",
      intro: [
        "한 대는 정상인데 DP로 연결한 두 번째 모니터만 감지되지 않는다면, PC 전체의 화면 미출력과는 점검 순서가 다릅니다. 각 모니터·케이블·포트를 단독으로 시험한 결과와 두 대를 함께 연결한 결과를 비교해야 합니다.",
      ...
      checks: [
        ...
        {
          title: "DP 입력 소스와 직접 연결 확인",
          why: "모니터 OSD의 입력 소스가 다르거나 허브·변환 젠더가 중간에 있으면 감지가 실패할 수 있습니다.",
          how: "모니터 메뉴에서 DP 또는 Auto를 선택하고, 도킹·KVM·허브·변환 젠더를 잠시 빼고 그래픽카드 DP 포트에 직접 연결해 비교하세요."
        },
        {
      ...
      examples: [
        "DP 모니터 한 대는 나오지만 보조 모니터만 '신호 없음'으로 표시됨",
