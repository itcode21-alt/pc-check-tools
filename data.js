window.SITE_DATA = {
  ...
  errorCodes: [
    ...
    },
    {
      ...
      detailPage: "error-code-0x0000009c.html", faq: [{"q": "게임할 때만 재부팅되면 그래픽카드 문제인가요?", "a": "이 문제(0x0000009C)는 그래픽 드라이버 문제일 가능성이 높지만 전원 공급이나 온도 문제와 겹치는 경우도 많아, 드라이버 재설치와 함께 온도·전력 상태도 함께 확인하는 것이 좋습니다."}, {"q": "전원 공급 장치(PSU) 용량이 부족한지 어떻게 확인하나요?", "a": "그래픽카드·CPU 권장 용량의 합을 계산해 PSU 정격 용량과 비교하면 부족 여부를 가늠할 수 있습니다."}], decisionSteps: [{"condition": "오버클럭 해제 후 재현되지 않을 때", "action": "오버클럭을 다시 적용하지 말고 안정적인 기본 설정을 유지하세요."}, {"condition": "온도가 높게 측정될 때", "action": "쿨링 시스템(써멀 재도포, 팬 청소) 점검을 우선하세요."}, {"condition": "메모리 진단·재장착 후에도 반복될 때", "action": "CPU나 메인보드 자체의 물리적 결함 가능성이 있으므로 다른 시스템에서 CPU 테스트를 고려하세요."}],
      relatedSymptom: "hardware-gaming-reboot.html",
    ...
    },
    {
      ...
      detailPage: "error-code-device-manager-code-45.html", faq: [{"q": "블루투스 장치가 안 잡히면 장치를 바꿔야 하나요?", "a": "이 문제(코드 45)는 장치보다 어댑터 드라이버나 페어링 기록 문제인 경우가 많아, 장치 교체 전에 드라이버 재설치와 페어링 재등록을 먼저 시도해보세요."}], decisionSteps: [{"condition": "연결·재연결 시 항목이 정상으로 바뀔 때", "action": "별다른 조치가 필요 없는 정상 동작입니다. 장치를 분리하면 회색으로 표시되는 것이 정상입니다."}, {"condition": "숨겨진 장치를 지워도 계속 다시 생길 때", "action": "어댑터·컨트롤러 드라이버나 절전 설정을 의심하고 4·5번 단계를 진행하세요."}, {"condition": "특정 포트에서만 반복될 때", "action": "장치보다 그 포트나 허브의 접촉·전력 공급 문제일 가능성이 큽니다."}],
      relatedSymptom: "hardware-bluetooth-not-found.html",
