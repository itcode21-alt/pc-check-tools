window.SITE_DATA = {
  ...
  errorCodes: [
    ...
    },
    {
      ...
      detailPage: "error-code-0x000000b4.html", decisionSteps: [{"condition": "드라이버 업데이트로 해결될 때", "action": "재발 여부만 지켜보면 됩니다."}, {"condition": "온도가 높게 측정될 때", "action": "쿨링 점검을 우선하세요."}, {"condition": "DDU 클린 재설치로도 해결되지 않을 때", "action": "그래픽카드 자체 결함 가능성이 있으므로 다른 시스템에서 테스트해보세요."}],
      relatedSymptom: "hardware-gaming-reboot.html",
    ...
    },
    {
      ...
      detailPage: "error-code-device-manager-code-1.html", decisionSteps: [{"condition": "드라이버 업데이트만으로 해결될 때", "action": "재발 여부만 지켜보면 됩니다."}, {"condition": "공식 드라이버 재설치로 해결될 때", "action": "재발 여부만 지켜보면 됩니다."}, {"condition": "USB 포트 변경으로도 해결되지 않을 때", "action": "드라이버를 완전히 제거한 뒤 재설치하는 방법을 시도하세요."}],
