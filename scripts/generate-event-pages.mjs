...
const eventCopy = {
  "9": {
    file: "event-disk-9.html",
    title: "Disk 이벤트 9 컨트롤러 오류 점검",
    eyebrow: "저장장치",
    interpretation: "9는 디스크 드라이버가 \\Device\\Harddisk 장치에서 컨트롤러 오류를 감지했다는 기록입니다. 이벤트 11과 원본이 같은 계열이라, 어떤 장치 번호에서 반복되는지부터 특정해야 합니다.",
    additional: "외장 드라이브나 카드리더처럼 연결이 잦은 장치에서만 나타난다면 케이블·포트·전원 공급 문제일 가능성이 크고, 내장 저장장치에서 반복된다면 컨트롤러나 저장장치 자체 상태를 우선 의심하세요.",
    faqs: [
      { q: "9가 뜨면 디스크가 곧 고장 나나요?", a: "단정할 수는 없습니다. 연결 불량으로도 자주 발생하므로, 먼저 케이블·포트를 바꿔 재현되는지 확인한 뒤에도 반복되면 저장장치 자체 문제를 의심하세요." },
      { q: "이벤트 11과 함께 나타나면 더 심각한 건가요?", a: "네, 9와 11이 같은 디스크 번호에서 함께 반복된다면 단순 연결 문제보다 저장장치나 컨트롤러의 실제 이상일 가능성이 높아 데이터 백업을 우선해야 합니다." }
    ]
  },
  "11": {
    file: "event-disk-11.html",
    title: "Disk 이벤트 11 디스크 오류 점검",
    eyebrow: "저장장치",
    interpretation: "11은 디스크 컨트롤러가 디스크 장치에서 오류를 감지했다는 기록으로, 이벤트 9보다 한 단계 더 명확한 오류 신호로 분류됩니다. 이벤트 7과 함께 같은 디스크에서 반복된다면 저장장치 이상으로 봐야 합니다.",
    additional: "부팅 중이나 파일 복사처럼 디스크 접근이 많은 작업에서 반복된다면, 재현 조건을 기록해두고 제조사 진단 도구로 SMART 상태를 함께 확인하는 것이 원인을 좁히는 데 도움이 됩니다.",
    faqs: [
      { q: "11만 있고 7은 없는데 괜찮은가요?", a: "11 단독이라면 당장 심각하지 않을 수 있지만, 반복 빈도가 늘어나는 추세라면 이벤트 7의 발생 여부를 계속 지켜봐야 합니다." },
      { q: "SATA 케이블을 바꿨는데도 반복되면 어떻게 하나요?", a: "케이블·포트 교체 후에도 같은 디스크 번호에서 반복된다면 저장장치 자체의 결함일 가능성이 크므로 제조사 진단 도구로 건강 상태를 확인하고 백업을 우선하세요." }
    ]
  },
  "50": {
    file: "event-ntfs-50.html",
    title: "Ntfs 이벤트 50 지연 쓰기 실패 점검",
    eyebrow: "저장장치",
    interpretation: "50은 NTFS가 저장장치에 데이터를 쓰려다 실패한 '지연 쓰기 실패' 기록입니다. 쓰기 요청이 즉시 실패한 것이 아니라 지연되다가 실패했다는 점에서 연결 안정성이나 전원 관리 설정을 먼저 의심해야 합니다.",
    additional: "외장 드라이브에서 나타난다면 절전으로 인한 USB 선택적 절전 설정을, 내장 저장장치라면 응답 시간 초과와 SMART 상태를 함께 확인하세요.",
    faqs: [
      { q: "50이 뜨면 저장한 파일이 손상됐나요?", a: "가능성이 있습니다. 이 이벤트가 발생한 직후 저장한 파일은 정상적으로 기록됐는지 다시 열어 확인하는 것이 안전합니다." },
      { q: "외장 드라이브에서만 반복되는데 원인이 뭘까요?", a: "USB 전원 관리 설정으로 장치가 절전에 들어갔다가 응답이 늦어지는 경우가 흔합니다. 장치 관리자에서 해당 드라이브의 절전 옵션을 꺼보세요." }
    ]
  },
  "57": {
    file: "event-ntfs-57.html",
    title: "Ntfs 이벤트 57 메타데이터 쓰기 실패 점검",
    eyebrow: "저장장치",
    interpretation: "57은 NTFS 파일 시스템 메타데이터 쓰기 자체가 실패했다는 기록으로, 이벤트 50보다 더 근본적인 파일 시스템 무결성 문제를 가리킵니다. 반복된다면 데이터 백업을 최우선으로 두어야 합니다.",
    additional: "이벤트 55(파일 시스템 손상)와 같은 시각에 나타난다면 저장장치 상태가 이미 상당히 나빠졌을 가능성이 크므로, chkdsk 실행보다 백업을 먼저 완료하는 순서를 지키세요.",
    faqs: [
      { q: "57과 55 중 무엇을 먼저 봐야 하나요?", a: "둘 다 파일 시스템 손상과 관련되므로 우선순위보다 같은 시각에 함께 나타나는지를 확인하는 것이 중요합니다. 함께 반복된다면 저장장치 교체를 고려할 시점입니다." },
      { q: "chkdsk를 실행하면 복구되나요?", a: "일부 손상은 복구될 수 있지만, 물리적으로 불안정한 저장장치에서는 검사 자체가 추가 부담이 될 수 있으므로 실행 전 반드시 중요 데이터를 먼저 백업하세요." }
    ]
  },
  "157": {
    file: "event-disk-157.html",
    title: "Disk 이벤트 157 갑작스러운 장치 제거 점검",
    eyebrow: "저장장치",
    interpretation: "157은 운영 체제가 종료 절차 없이 디스크가 갑자기 제거됐음을 감지한 기록입니다. 대부분 외장 드라이브를 안전 제거 없이 뽑았거나 연결이 순간적으로 끊어졌을 때 남습니다.",
    additional: "내장 드라이브에서 나타난다면 안전 제거와는 무관하므로 전원 공급이나 SATA·M.2 연결 접촉 불량, 배터리 방전으로 인한 강제 종료 여부를 확인하세요.",
    faqs: [
      { q: "안전 제거를 했는데도 157이 남나요?", a: "안전 제거 절차를 거쳤다면 보통 남지 않습니다. 남아 있다면 제거 도중 이미 연결이 끊어졌거나 절차가 완료되기 전에 뽑았을 가능성이 있습니다." },
      { q: "내장 드라이브에서 반복되면 무엇을 확인해야 하나요?", a: "외장 장치가 아니라면 전원 공급 불안정이나 SATA·M.2 슬롯 접촉 불량을 의심해야 합니다. 케이블과 전원 커넥터 상태를 먼저 점검하세요." }
    ]
  },
  "4625": {
    file: "event-security-4625.html",
    title: "Security 4625 로그온 실패 점검",
    eyebrow: "보안·계정",
    interpretation: "4625는 계정 로그온이 실패했다는 기록입니다. 실패 이유 코드와 로그온 유형(대화형/원격/네트워크)을 함께 확인해야 단순 오타인지 외부 접근 시도인지 구분할 수 있습니다.",
    additional: "본인이 시도하지 않은 시각에 특정 계정만 짧은 간격으로 반복 실패한다면, 원격 데스크톱이나 공유 폴더처럼 외부에서 접근 가능한 경로가 노출되어 있는지 함께 확인하세요.",
    faqs: [
      { q: "4625가 가끔 있는 건 정상인가요?", a: "본인이 비밀번호를 잘못 입력했거나 CapsLock·한영 전환 상태였다면 정상적으로 남는 기록입니다. 특정 계정에서 짧은 시간에 몰아서 반복되는지가 핵심 판단 기준입니다." },
      { q: "소스 IP가 낯선 외부 주소인데 어떻게 해야 하나요?", a: "원격 데스크톱이나 공유 서비스가 인터넷에 노출되어 있는지 확인하고, 불필요하다면 포트를 닫거나 방화벽 규칙을 강화하며 즉시 계정 비밀번호를 변경하세요." }
    ]
  },
  "4740": {
    file: "event-security-4740.html",
    title: "Security 4740 계정 잠금 점검",
    eyebrow: "보안·계정",
    interpretation: "4740은 계정이 잠겼다는 기록으로, 대부분 4625(로그온 실패)가 짧은 시간 동안 반복된 결과로 나타납니다. 호출자 PC 이름을 확인하면 어떤 장치가 반복 실패를 일으켰는지 특정할 수 있습니다.",
    additional: "비밀번호를 최근에 변경했다면, 자격 증명 관리자나 예약 작업, 모바일 메일 앱처럼 예전 비밀번호를 저장해 둔 곳이 자동으로 재시도하며 계정을 잠그는 경우가 흔합니다.",
    faqs: [
      { q: "비밀번호를 바꾼 적 없는데 계정이 잠겨요.", a: "다른 사람이 시도했을 수도 있지만, 우선 자격 증명 관리자와 여러 장치에 저장된 로그인 정보 중 오래된 비밀번호가 남아있는지부터 확인하세요." },
      { q: "호출자 PC 이름은 어디서 확인하나요?", a: "이벤트 뷰어의 일반 탭 본문 또는 자세히 탭의 XML 보기에서 Caller Computer Name 항목을 확인하면 잠금을 유발한 장치를 특정할 수 있습니다." }
    ]
  },
  "8193": {
    file: "event-vss-8193.html",
    title: "VSS 8193 볼륨 섀도 복사본 오류 점검",
    eyebrow: "백업·복원",
    interpretation: "8193은 볼륨 섀도 복사본 서비스(VSS)가 오류로 실패했다는 기록으로, 백업 소프트웨어 실행이나 시스템 복원 지점 생성 중에 주로 나타납니다. 이벤트 설명에 표시된 오류 코드로 원인 범위를 좁혀야 합니다.",
    additional: "디스크 여유 공간이 충분한지 먼저 확인하고, 백업 소프트웨어를 여러 개 함께 쓰고 있다면 VSS 작성기 충돌 가능성도 열어두세요.",
    faqs: [
      { q: "8193이 뜨면 백업이 실패한 건가요?", a: "백업 소프트웨어나 시스템 복원이 VSS를 사용하는 도중 실패했을 가능성이 큽니다. 백업 프로그램의 실행 로그를 함께 확인해 실제로 완료됐는지 확인하세요." },
      { q: "vssadmin list writers는 어떻게 실행하나요?", a: "관리자 권한 명령 프롬프트에서 vssadmin list writers를 입력하면 각 작성기의 상태가 표시됩니다. 오류 상태인 작성기가 있다면 관련 서비스를 재시작해보세요." }
    ]
  },
  "6013": {
    file: "event-eventlog-6013.html",
    title: "EventLog 6013 시스템 가동 시간 확인",
    eyebrow: "정보성 기록",
    interpretation: "6013은 시스템이 부팅된 이후 경과한 시간(가동 시간)을 초 단위로 기록하는 정보성 이벤트입니다. 매일 자동으로 생성되며 그 자체는 문제를 의미하지 않습니다.",
    additional: "가동 시간이 예상보다 훨씬 짧게 반복해서 기록된다면 그 시점에 재부팅이 있었다는 뜻이므로, 같은 시각의 Kernel-Power 41이나 EventLog 6008 같은 종료 관련 이벤트를 함께 확인하세요.",
    faqs: [
      { q: "6013을 지금 당장 조치해야 하나요?", a: "아닙니다. 정보성 이벤트로 매일 자동 기록되며 그 자체로는 문제가 아닙니다. 가동 시간 값이 비정상적으로 짧게 반복될 때만 다른 이벤트와 함께 살펴보면 됩니다." },
      { q: "가동 시간이 짧으면 무엇을 의심해야 하나요?", a: "예상하지 못한 재부팅이 있었다는 뜻이므로 같은 날짜·시각의 Kernel-Power 41, WHEA, EventLog 6008 이벤트를 함께 확인해 재부팅 원인을 좁히세요." }
    ]
  },
  "1102": {
    file: "event-security-1102.html",
    title: "Security 1102 감사 로그 지움 점검",
    eyebrow: "보안·계정",
    interpretation: "1102는 보안 감사 로그가 지워졌다는 기록입니다. 관리자가 의도적으로 로그를 정리한 경우도 있지만, 침해 사고 뒤 흔적을 지우려는 시도로도 나타날 수 있어 누가·언제 지웠는지 반드시 확인해야 합니다.",
    additional: "본인이나 IT 담당자가 지운 기억이 없는 상태에서 이 이벤트가 나타났다면, 삭제 직전 로그인 기록과 실행된 프로그램 흔적을 다른 로그(예: 시스템 로그)에서 함께 대조해보세요.",
    faqs: [
      { q: "1102가 있으면 무조건 해킹인가요?", a: "아닙니다. 관리 목적의 정상적인 로그 정리에서도 남습니다. 다만 지운 기억이 없다면 이벤트의 계정 이름과 시각을 확인해 본인·IT 담당자의 작업이 맞는지부터 확인하세요." },
      { q: "로그가 지워지면 이전 기록은 복구할 수 없나요?", a: "기본적으로 지워진 이벤트 로그 자체는 복구가 어렵습니다. 정기적으로 로그를 파일로 내보내 보관하면 이런 상황에서도 이전 기록을 확인할 수 있습니다." }
    ]
  },
  "1074": {
