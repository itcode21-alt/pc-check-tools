...
const gameCopy = {
  ...
  steam: {
    intro: "Steam은 게임 설치·업데이트·실행을 하나의 클라이언트가 관리하므로, 파일 손상과 저장장치 문제, 네트워크·방화벽 문제를 구분해야 합니다. 아래 항목은 Steam 자체 메시지와 공식 지원 절차를 기준으로 정리했습니다.",
    faqs: [
      { q: "무결성 검사에서 일부 파일이 실패하면 게임이 고장 난 건가요?", a: "Steam 공식 안내처럼 로컬 설정 파일이 교체 대상이 아니어서 일부 파일이 실패로 표시되는 경우가 있습니다. 검사가 끝난 뒤 실제 게임 실행과 업데이트가 되는지 함께 확인하세요." },
      { q: "디스크 쓰기 오류가 나오면 바로 SSD를 교체해야 하나요?", a: "아닙니다. 파일 잠금이나 백신 격리도 흔한 원인입니다. 여러 게임에서 반복되거나 SMART 경고·파일 복사 오류가 동반될 때 백업과 저장장치 점검을 우선하세요." },
      { q: "Steam 폴더를 통째로 삭제해도 되나요?", a: "게임 파일과 세이브가 영향을 받을 수 있으므로 먼저 steamapps와 필요한 설정을 백업하세요. 공식 안내에서 제시한 범위와 순서를 확인한 뒤 런처 파일만 복구하는 방식이 안전합니다." }
    ]
  },
  battlenet: {
    intro: "Battle.net 오류는 런처 자체의 업데이트 문제와 개별 게임 파일 문제를 먼저 분리해야 합니다. 오류 코드가 있는 경우 코드를 그대로 기록하고, 복구 도구를 먼저 실행한 뒤 재설치 여부를 판단하는 것이 안전합니다.",
    faqs: [
      { q: "BLZBNTAGT00000840이면 게임을 바로 삭제해야 하나요?", a: "아닙니다. 블리자드 공식 절차는 재시작, 복구 도구, 보안 프로그램 확인, 권한 점검을 먼저 안내합니다. 게임 삭제는 앞 단계가 실패했을 때 마지막 수단으로 판단하세요." },
      { q: "백신을 꺼두면 해결되나요?", a: "원인 확인을 위해 짧게 차단 기록을 확인할 수는 있지만, 장시간 비활성화하는 방법은 권장하지 않습니다. 원인을 확인한 뒤 필요한 파일만 신뢰 목록에 추가하고 보호 기능을 다시 켜세요." }
    ]
  },
  common: {
    intro: "특정 게임 하나가 아니라 여러 게임에서 화면 전환·Alt+Tab·로딩 복귀 직후 화면이 꺼지는 증상을 다룹니다. 게임 파일 문제와 그래픽 드라이버 TDR, GPU 전원·발열 문제를 구분하는 순서가 핵심입니다.",
    faqs: [
      { q: "화면이 꺼지면 그래픽카드가 고장 난 건가요?", a: "반드시 그렇지는 않습니다. 드라이버가 일시적으로 재설정되는 TDR, 케이블·모니터·오버레이 문제도 같은 증상을 만들 수 있습니다. 여러 게임과 바탕화면에서도 반복되는지로 범위를 좁혀보세요." },
      { q: "TDR 레지스트리 값을 늘리면 해결되나요?", a: "응답 시간만 늘려 증상을 늦출 수 있어 권장되는 첫 해결책이 아닙니다. 드라이버·발열·전원·오버클럭 원인을 먼저 점검하고 변경 전 복원 지점을 준비하세요." },
      { q: "Win+Ctrl+Shift+B 단축키는 안전한가요?", a: "Windows가 그래픽 드라이버를 재설정하는 기본 단축키입니다. 화면이 잠시 깜빡이거나 알림음이 난 뒤 복구되는지 확인하는 용도로 사용할 수 있습니다." }
    ]
  }
};
