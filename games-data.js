...
,
  ...
  {
    id: "lol-ingame-freeze",
    game: "리그 오브 레전드",
    category: "그래픽/성능",
    errorCode: "인게임 화면 멈춤(크래시 아님)",
    title: "게임 중 화면이 멈추고 조작이 안 되지만 클라이언트가 꺼지지는 않는 문제",
    overview: "챔피언 선택이 아닌 실제 게임 진행 중에 화면이 완전히 멈춰 조작이 반응하지 않는 증상입니다. lol-graphics-texture-glitch(화면이 순간 깨졌다가 돌아오는 문제)와 달리 화면이 정상으로 돌아오지 않고 계속 멈춰 있으며, 그렇다고 클라이언트가 강제 종료되지도 않는 것이 특징입니다.",
    causes: [
      "그래픽 드라이버가 게임 도중 응답 없음 상태에 빠졌지만 완전히 크래시하지는 않는 경우",
      "백그라운드에서 실행 중인 화면 녹화·오버레이(디스코드, 지포스 익스피리언스 등) 프로그램과의 충돌",
      "저사양 PC에서 순간적으로 CPU·메모리 사용량이 튀며 렌더링 스레드가 멈추는 경우",
      "인터넷 연결은 살아있지만 게임 서버와의 동기화가 순간적으로 끊겨 화면 업데이트만 멈추는 경우"
    ],
    solutions: [
      "**1단계 - 강제 복귀 시도:**\n① Alt+Tab으로 다른 창을 열었다가 게임으로 돌아와 반응이 오는지 확인\n② 반응이 없으면 작업 관리자(Ctrl+Shift+Esc)로 리그 오브 레전드 프로세스가 \"응답 없음\" 상태인지 확인",
      "**2단계 - 오버레이 프로그램 종료:**\n① 디스코드·지포스 익스피리언스·화면 녹화 프로그램의 인게임 오버레이 기능을 꺼본 뒤 재현 여부 확인",
  ...
  {
    id: "common-fullscreen-optimization-fps-drop",
    game: "PC 게임 공통",
    category: "그래픽/성능",
    errorCode: "전체 화면 최적화로 인한 프레임 저하",
    title: "특정 게임에서만 프레임이 고정되거나 뚝뚝 끊기는 문제",

    overview: "Windows 11의 '전체 화면 최적화'·'창이 있는 게임에 대한 최적화' 기능은 대부분 게임에 도움이 되지만, 일부 게임·그래픽 드라이버 조합에서는 오히려 프레임이 고정되거나 끊기는 원인이 됩니다. 특정 게임 하나에서만 증상이 나타난다면 이 설정부터 의심해볼 만합니다.",

    causes: [
      "전체 화면 최적화가 게임의 자체 전체화면 처리 방식과 충돌하는 경우",
      "다중 평면 오버레이(MPO)가 특정 그래픽 드라이버·모니터 조합에서 버벅임을 일으키는 경우",
      "게임 바(Game Bar) 오버레이가 백그라운드에서 프레임 드랍을 유발하는 경우"
    ],

    solutions: [
      "**1단계 - 전체 화면 최적화 끄기:**\n① 게임 실행 파일(.exe) 우클릭 → 속성 → 호환성 탭\n② '전체 화면 최적화 사용 안 함'에 체크 후 적용",
      "**2단계 - 창이 있는 게임 최적화 확인:**\n① 설정 → 시스템 → 디스플레이 → 그래픽\n② '창이 있는 게임에 대한 최적화' 스위치를 껐다 켜보며 프레임 차이 비교",
      "**3단계 - 게임 바 끄기:**\n① 설정 → 게임 → 게임 바에서 기능을 꺼서 오버레이가 프레임에 영향을 주는지 배제",
      "**4단계 - 그래픽 드라이버 재설치:**\n① 위 설정으로도 해결되지 않으면 그래픽 드라이버를 최신 버전으로 클린 설치"
    ],

    officialSource: { title: "Microsoft 지원: Windows 11 창이 있는 게임에 대한 최적화", url: "https://support.microsoft.com/ko-kr/windows/windows-11-%EC%B0%BD%EC%9D%B4-%EC%9E%88%EB%8A%94-%EA%B2%8C%EC%9E%84%EC%97%90-%EB%8C%80%ED%95%9C-%EC%B5%9C%EC%A0%81%ED%99%94-3f006843-2c7e-4ed0-9a5e-f9389e535952" },
    communityReports: [],
    keywords: ["게임", "프레임 드랍", "전체화면 최적화", "MPO", "버벅임"],
    lastUpdated: "2026-09-22"
  },

  {
    id: "common-xbox-game-bar-conflict",
    game: "PC 게임 공통",
    category: "그래픽/실행",
    errorCode: "Xbox Game Bar(Win+G) 오버레이 충돌",
    title: "Win+G 게임 바 오버레이 때문에 게임 중 입력이 씹히거나 프레임이 떨어지는 문제",

    overview: "Windows에 내장된 Xbox Game Bar는 녹화·스크린샷을 위해 여러 게임에서 백그라운드로 함께 동작합니다. 다른 오버레이 프로그램과 함께 켜져 있으면 단축키가 충돌하거나, 오버레이 자체가 프레임을 갉아먹는 경우가 있습니다.",

    causes: [
      "다른 프로그램(스트리밍·녹화 툴)과 Win+G 단축키가 겹치는 경우",
      "여러 오버레이 프로그램이 동시에 실행돼 서로 간섭하는 경우",
      "게임 바 백그라운드 프로세스가 멈춘 상태로 남아있는 경우"
    ],

    solutions: [
      "**1단계 - 게임 바 설정 확인:**\n① 설정 → 게임 → 게임 바에서 기능이 켜져 있는지, 다른 프로그램과 단축키가 겹치지 않는지 확인",
      "**2단계 - 다른 오버레이 정리:**\n① GeForce Experience·Discord·백신 등 다른 오버레이 프로그램을 하나씩 꺼서 어떤 조합에서 충돌하는지 확인",
      "**3단계 - 게임 바 자체를 끄기:**\n① 녹화·스크린샷 기능이 필요 없다면 설정 → 게임 → 게임 바를 꺼서 오버레이 자체를 비활성화",
      "**4단계 - 백그라운드 프로세스 재시작:**\n① 작업 관리자에서 Xbox 관련 프로세스를 종료한 뒤 재부팅"
    ],

    officialSource: { title: "Xbox 지원: Windows에서 Game Bar 문제 해결", url: "https://support.xbox.com/ko-KR/help/games-apps/game-setup-and-play/troubleshoot-game-bar-windows" },
    communityReports: [],
    keywords: ["게임", "Xbox Game Bar", "Win+G", "오버레이 충돌", "입력 씹힘"],
    lastUpdated: "2026-09-22"
  },

  {
    id: "common-overlay-crash-discord-steam",
    game: "PC 게임 공통",
    category: "그래픽/실행",
    errorCode: "디스코드·스팀 오버레이로 인한 게임 튕김",
    title: "디스코드나 스팀 오버레이를 켜두면 게임이 튕기거나 멈추는 문제",

    overview: "음성 채팅이나 인게임 상점을 표시하는 오버레이는 게임 그래픽 위에 별도 레이어를 그리는 방식이라, 게임·그래픽 드라이버와의 궁합에 따라 크래시로 이어질 수 있습니다. 디스코드 공식 안내에 따르면 오버레이는 '트루 전체화면'에서는 아예 지원되지 않고 창모드·테두리 없는 창모드에서만 동작합니다.",

    causes: [
      "게임이 '트루 전체화면'으로 실행 중이라 오버레이와 렌더링 방식이 충돌하는 경우",
      "디스코드 하드웨어 가속과 게임의 그래픽 API가 같은 GPU 자원을 두고 충돌하는 경우",
      "여러 오버레이(디스코드+스팀+GeForce 등)가 동시에 게임에 주입되는 경우"
    ],

    solutions: [
      "**1단계 - 화면 모드 확인:**\n① 게임을 트루 전체화면 대신 '테두리 없는 창모드'로 바꿔 오버레이가 정상 지원되는 모드인지 확인",
      "**2단계 - 디스코드 오버레이 끄기:**\n① 디스코드 설정 → 오버레이(Overlay) → 게임 내 오버레이 활성화를 꺼서 재현 여부 확인",
      "**3단계 - 하드웨어 가속 끄기:**\n① 디스코드 설정 → 고급(Advanced) → 하드웨어 가속을 끈 뒤 재시도",
      "**4단계 - 스팀 오버레이도 함께 확인:**\n① 스팀 라이브러리에서 게임 우클릭 → 속성 → '게임 중 Steam 오버레이 사용'을 꺼서 스팀 오버레이와의 충돌인지 구분"
    ],

    officialSource: { title: "Discord 공식 안내: Game Overlay 알려진 문제", url: "https://support.discord.com/hc/en-us/articles/25289844838551--Known-Issue-Game-Overlay" },
    communityReports: [],
    keywords: ["게임", "디스코드 오버레이", "스팀 오버레이", "게임 튕김", "크래시"],
    lastUpdated: "2026-09-22"
  },

  {
    id: "common-mouse-cursor-second-monitor",
    game: "PC 게임 공통",
    category: "기타",
    errorCode: "마우스 커서가 다른 모니터로 빠짐",
    title: "듀얼 모니터 환경에서 게임 중 마우스 커서가 다른 화면으로 넘어가는 문제",

    overview: "듀얼 모니터를 쓸 때 일부 게임은 마우스 입력을 창 안에 가두지 못해, 빠르게 움직이거나 화면 가장자리를 조준할 때 커서가 옆 모니터로 빠져나가 조작이 끊기는 문제입니다. 창모드·테두리 없는 창모드에서 특히 자주 발생합니다.",

    causes: [
      "게임이 창모드·테두리 없는 창모드로 실행돼 마우스를 창 안에 가두지 않는 경우",
      "Windows 디스플레이 설정이 '여러 디스플레이에 표시'로 되어 있어 커서가 자유롭게 이동하는 경우",
      "게임 내 마우스 잠금(캡처) 옵션이 꺼져 있는 경우"
    ],

    solutions: [
      "**1단계 - 게임 내 설정 확인:**\n① 게임 옵션에서 '마우스 잠금'·'창에 커서 가두기' 같은 항목이 있으면 켜기",
      "**2단계 - 전체화면으로 전환:**\n① 가능하면 트루 전체화면으로 실행해 커서가 다른 모니터로 넘어가지 않는지 확인",
      "**3단계 - 보조 모니터 임시 끄기:**\n① Win+P를 눌러 'PC 화면만'을 선택하면 출력이 주 모니터로 모여 커서 이탈을 즉시 막을 수 있습니다(게임 종료 후 원래대로 되돌리기)",
      "**4단계 - 그래픽 드라이버 업데이트:**\n① 위 방법으로도 반복되면 그래픽 드라이버를 최신 버전으로 업데이트"
    ],

    officialSource: { title: "Microsoft Q&A: 게임 중 마우스가 다른 모니터로 이동하는 문제", url: "https://learn.microsoft.com/en-us/answers/questions/3952899/my-mouse-is-going-to-my-other-monitor-while-i-am-i" },
    communityReports: [],
  ...
  {
    id: "poe2-memory-leak-fps-drop",
    game: "패스 오브 엑자일 2",
    category: "그래픽/성능",
    errorCode: "장시간 플레이 시 프레임 저하",
    title: "플레이 시간이 길어질수록 프레임이 점점 떨어지는 문제 (메모리 누수)",

    overview: "접속 직후에는 정상이던 프레임이 오래 플레이할수록 점점 낮아지는 문제로, 균열·환영·심연 같은 콘텐츠가 몰린 맵에서 몬스터가 대량으로 몰릴 때 특히 심해집니다. 재접속하면 일시적으로 프레임이 회복되는 것이 특징입니다.",

    causes: [
      "장시간 플레이 시 누적되는 메모리 누수(memory leak)",
      "여러 이벤트 콘텐츠(균열·환영·심연 등)와 몬스터 스폰이 동시에 몰리는 경우"
    ],

    solutions: [
      "**1단계 - 재접속으로 임시 회복:**\n① 프레임이 눈에 띄게 떨어지면 게임을 완전히 종료했다가 재접속해 임시로 회복",
      "**2단계 - 이벤트 콘텐츠 중첩 줄이기:**\n① 균열·환영·심연 등 여러 콘텐츠를 동시에 몰아서 진행하지 않고 나눠서 진행해 재현 빈도 비교",
      "**3단계 - 그래픽 옵션 낮추기:**\n① 파티클·이펙트 품질을 낮춰 메모리 사용량 자체를 줄이기",
      "**4단계 - 공식 게시판에 사양과 함께 제보:**\n① 반복되면 플레이 시간, 사양, 발생 콘텐츠를 함께 공식 게시판에 제보"
    ],

    officialSource: { title: "Path of Exile 2 카카오게임즈 공식 홈페이지", url: "https://poe2.kakaogames.com/home" },
    communityReports: [
      { summary: "인벤·디시인사이드 등 커뮤니티에서 사양과 무관하게 플레이 시간이 길어질수록 프레임이 120대에서 30~40대까지 떨어진다는 보고가 다수 확인됩니다. 균열·환영·심연이 겹치는 구간에서 특히 심하다는 공통된 경험이 보고됩니다.", url: "https://www.inven.co.kr/board/poe/6317/18494" }
    ],
  ...
  {
    id: "starcraft2-arcade-crash",
    game: "스타크래프트2",
    category: "접속/서버",
    errorCode: "사용자지정 아케이드 입장 시 튕김",
    title: "KR 서버에서 사용자지정 아케이드 맵에 입장하면 게임이 튕기는 문제",

    overview: "배틀넷 로그인과 일반 대전은 정상인데, 사용자지정(아케이드) 게임 목록에서 특정 맵에 입장할 때만 게임이 강제 종료되는 문제입니다. 한글 클라이언트에서 반복 보고되고 있으며 재설치로도 해결되지 않는 경우가 많습니다.",

    causes: [
      "일부 아케이드 맵이 한국어 클라이언트에서 튕기는 알려진 호환성 문제를 갖고 있는 경우",
      "표시 언어가 한국어일 때만 발생하고 영어로 바꾸면 재현되지 않는 사례가 다수 보고됨",
      "Battle.net 캐시에 남은 손상된 맵 데이터"
    ],

    solutions: [
      "**1단계 - 표시 언어를 영어로 임시 전환:**\n① Battle.net 앱에서 톱니바퀴(설정) → 게임 설정 → 스타크래프트 II 선택\n② 게임 언어(텍스트) 또는 Battle.net 표시 언어를 영어(English)로 바꾼 뒤 재접속\n③ 같은 맵에 다시 입장해 튕김이 재현되는지 확인 — 언어를 바꾸면 재현되지 않는다는 보고가 많습니다",
      "**2단계 - 스캔 및 복구:**\n① Battle.net 런처에서 스타크래프트 II 선택 → 옵션(톱니바퀴) → '스캔 및 복구' 실행",
      "**3단계 - 문제 맵 회피 및 제보:**\n① 특정 맵에서만 반복된다면 해당 맵 이름과 재현 조건을 적어 공식 포럼 버그 제보 게시판에 신고"
    ],

    officialSource: { title: "Blizzard StarCraft II 공식 포럼: 아케이드 튕김 버그 제보", url: "https://kr.forums.blizzard.com/ko/sc2/t/kr%EC%84%9C%EB%B2%84-%EC%82%AC%EC%9A%A9%EC%9E%90%EC%A7%80%EC%A0%95-%EC%95%84%EC%BC%80%EC%9D%B4%EB%93%9C-%EC%9E%85%EC%9E%A5%EC%8B%9C-%EC%A7%80%EC%86%8D%EC%A0%81%EC%9C%BC%EB%A1%9C-%ED%8A%95%EA%B8%B0%EB%8A%94-%ED%98%84%EC%83%81/4636" },
    communityReports: [],
    keywords: ["스타크래프트2", "스타2", "아케이드 튕김", "커스텀 게임 튕김"],
    lastUpdated: "2026-09-22"
  },

  {
    id: "starcraft2-game-data-redownload",
    game: "스타크래프트2",
    category: "설치/패치",
    errorCode: "게임 데이터 준비중 반복 재다운로드",
    title: "접속할 때마다 '게임 데이터 준비중'과 함께 대용량 재다운로드가 발생하는 문제",

    overview: "게임을 켤 때마다 수백 MB 단위의 데이터를 다시 받아야 하고, 다운로드가 끝난 뒤에도 한국어 텍스트가 적용되지 않는 문제가 공식 포럼에 보고돼 있습니다. 재부팅이나 PC 재설치로도 해결되지 않았다는 사례가 있어, 클라이언트 쪽 원인이 아직 명확히 확정되지 않은 이슈입니다.",

    causes: [
      "Battle.net 캐시나 언어팩 데이터가 매번 최신 버전으로 인식되지 않는 클라이언트 쪽 동기화 오류로 추정(공식 원인 미확정)",
      "일부 사용자는 재부팅·재설치 후에도 재현된다고 보고 — 로컬 설정만의 문제가 아닐 가능성"
    ],

    solutions: [
      "**1단계 - 스캔 및 복구:**\n① Battle.net 런처에서 스타크래프트 II 옵션 → '스캔 및 복구' 실행 후 재시도",
      "**2단계 - 언어 설정 재확인:**\n① Battle.net 게임 설정에서 텍스트 언어를 한국어가 아닌 언어로 바꿨다가 다시 한국어로 저장 후 재시작",
      "**3단계 - 공식 포럼에 재현 조건 제보:**\n① 현재 알려진 완전한 해결책이 없는 이슈이므로, 재다운로드 용량과 발생 시점을 적어 공식 버그 제보 게시판에 등록해두면 대응 현황을 추적하기 쉽습니다"
    ],

    officialSource: { title: "Blizzard StarCraft II 공식 포럼: 실행 시 게임 데이터 준비중 재다운로드 제보", url: "https://kr.forums.blizzard.com/ko/sc2/t/%EC%8A%A4%ED%83%80%ED%81%AC%EB%9E%98%ED%94%84%ED%8A%B82-%EC%8B%A4%ED%96%89-%EA%B2%8C%EC%9E%84-%EB%8D%B0%EC%9D%B4%ED%84%B0-%EC%A4%80%EB%B9%84%EC%A4%91/6818" },
    communityReports: [],
    keywords: ["스타크래프트2", "스타2", "게임 데이터 준비중", "재다운로드", "한국어 안됨"],
    lastUpdated: "2026-09-22"
  },

  {
    id: "starcraft2-no-sound",
    game: "스타크래프트2",
    category: "그래픽/성능",
    errorCode: "소리가 들리지 않음",
    title: "게임 실행 중 효과음·음성이 전혀 들리지 않는 문제",

    overview: "다른 프로그램의 소리는 정상인데 스타크래프트 II에서만 소리가 나지 않는 경우로, 대부분 Windows 또는 게임 내 오디오 장치 설정이 원인입니다. 블루투스 헤드폰 사용 중에는 별도로 알려진 충돌도 있습니다.",

    causes: [
      "Windows 소리 설정에서 출력 장치가 실제 사용 중인 장치와 다르게 지정된 경우",
      "게임 내 오디오 설정의 출력 장치·음소거 상태가 Windows 설정과 어긋난 경우",
      "블루투스 헤드폰 연결 중 게임이 모든 소리를 독점해 다른 장치로 출력이 전환되지 않는 경우"
    ],

    solutions: [
      "**1단계 - 재부팅 후 확인:**\n① 다른 문제 해결 전에 PC를 재시작해 일시적 오디오 드라이버 문제인지 먼저 배제",
      "**2단계 - Windows 소리 설정 확인:**\n① 작업 표시줄 스피커 아이콘에서 출력 장치가 올바른지, 음량이 0이 아닌지 확인",
      "**3단계 - 게임 내 오디오 설정 확인:**\n① 옵션 → 소리에서 출력 장치와 음량, 음소거 여부를 다시 확인",
      "**4단계 - 다른 하드웨어로 교차 확인:**\n① Windows/게임 설정이 모두 정상이면 다른 프로그램에서 같은 헤드폰·스피커로 소리가 나는지 확인해 하드웨어 문제인지 구분",
      "**5단계 - 블루투스 헤드폰이라면:**\n① 게임 실행 중 블루투스 헤드폰을 새로 연결하지 말고, 헤드폰을 먼저 연결·페어링한 뒤 게임을 실행"
    ],

    officialSource: { title: "Blizzard 공식 지원: 스타크래프트 II에서 소리가 들리지 않습니다", url: "https://kr.support.blizzard.com/ko/article/10998" },
    communityReports: [],
  ...
  {
    id: "specialforce-steam-epic-integrity",
    game: "스페셜포스",
    category: "실행/파일",
    errorCode: "Steam·Epic Games 버전 실행 불가",
    title: "Steam·Epic Games 버전에서 게임이 실행되지 않거나 시작 직후 크래시하는 문제",

    overview: "스페셜포스 리마스터는 넷마블 런처 외에 Steam·Epic Games 스토어로도 배포됩니다. 두 플랫폼 모두 실행 파일이나 설치 데이터 일부가 손상되면 클릭 후 아무 반응이 없거나 시작 직후 꺼지는 증상으로 나타납니다.",

    causes: [
      "설치 파일 다운로드 도중 일부가 누락되거나 손상된 경우",
      "업데이트 중 강제 종료로 패치 파일이 불완전하게 적용된 경우",
      "백신·보안 프로그램이 실행 파일을 차단하는 경우"
    ],

    solutions: [
      "**1단계(Steam) - 게임 파일 무결성 검사:**\n① 라이브러리에서 스페셜포스 리마스터 우클릭 → 속성\n② 설치된 파일 탭 → '게임 파일 무결성 검사' 실행",
      "**1단계(Epic Games) - 파일 인증:**\n① 라이브러리에서 게임 관리 → 인증 선택 후 완료까지 대기",
      "**2단계 - 보안 프로그램 예외 등록:**\n① 백신 프로그램에서 게임 설치 폴더와 실행 파일을 예외(허용) 목록에 추가",
      "**3단계 - 재설치:**\n① 위 방법으로도 실행되지 않으면 게임을 완전히 삭제한 뒤 재설치"
    ],

    officialSource: { title: "스페셜포스 리마스터 공식 고객지원(FAQ)", url: "https://sfr.dfl.co.kr/Customer" },
    communityReports: [],
    keywords: ["스페셜포스", "스팀", "에픽게임즈", "실행 안됨", "파일 무결성 검사"],
    lastUpdated: "2026-09-22"
  },

  {
    id: "specialforce-crash-ingame",
    game: "스페셜포스",
    category: "그래픽/실행",
    errorCode: "게임 튕김 현상",
    title: "플레이 도중 예고 없이 게임이 강제 종료되는 문제",

    overview: "매칭 중이나 플레이 도중 특정 상황 없이 게임이 갑자기 꺼지는 문제입니다. 그래픽 드라이버 호환성, 악성코드, 메모리 부족 등 여러 원인이 겹칠 수 있어 공식 FAQ도 단계적 점검을 권장합니다.",

    causes: [
      "그래픽 드라이버가 오래되었거나 최신 업데이트와 호환되지 않는 경우",
      "악성코드나 다른 프로그램과의 충돌",
      "실행 중 메모리 부족"
    ],

    solutions: [
      "**1단계 - 백신 정밀 검사:**\n① 백신 프로그램을 최신 버전으로 업데이트한 뒤 정밀 검사 실행",
      "**2단계 - 그래픽 드라이버 업데이트:**\n① 그래픽카드 제조사 홈페이지에서 최신 드라이버로 업데이트",
      "**3단계 - 백그라운드 프로그램 정리:**\n① 실행 중인 불필요한 프로그램을 종료해 여유 메모리 확보",
      "**4단계 - 재설치:**\n① 위 방법으로도 반복되면 게임을 재설치"
    ],

    officialSource: { title: "스페셜포스 리마스터 공식 고객지원(FAQ)", url: "https://sfr.dfl.co.kr/Customer" },
    communityReports: [],
    keywords: ["스페셜포스", "게임 튕김", "크래시"],
    lastUpdated: "2026-09-22"
  },

  {
    ...
    category: "서버/네트워크",
    errorCode: "렉(끊김) 현상",
    title: "플레이 중 화면이 끊기거나 입력이 늦게 반영되는 문제",

    overview: "총격전처럼 반응 속도가 중요한 FPS 특성상 렉은 곧바로 승패에 영향을 줍니다. PC 자원 부족인지 네트워크 지연인지를 먼저 구분하는 것이 첫 단계입니다.",

    causes: [
      "백그라운드에서 CPU·메모리를 많이 쓰는 프로그램이 함께 실행 중인 경우",
      "인터넷 회선 상태가 불안정한 경우",
      "공유기 펌웨어가 오래되어 처리 성능이 떨어진 경우"
    ],

    solutions: [
      "**1단계 - 자원 점유 프로그램 확인:**\n① 작업 관리자에서 CPU·메모리 점유율이 높은 프로그램을 확인하고 불필요한 프로그램 종료",
      "**2단계 - 인터넷 회선 점검:**\n① 다른 기기·사이트에서도 속도가 느린지 확인해 PC만의 문제인지 회선 문제인지 구분",
      "**3단계 - 공유기 재시작·펌웨어 업데이트:**\n① 공유기를 재시작하고, 제조사 홈페이지에서 최신 펌웨어가 있는지 확인"
    ],
